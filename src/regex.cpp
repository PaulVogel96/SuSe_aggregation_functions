...
token lexer::tokenize_next() {
  ...
  case '(':
    return token{token_type::open_parenthesis, symbol, input_position_ - 1};
  case ')':
    return token{token_type::close_parenthesis, symbol, input_position_ - 1};
  case '.':
    return token{token_type::wildcard, symbol, input_position_ - 1};
  case '|':
    return token{token_type::alternative, symbol, input_position_ - 1};
  case '*':
    return token{token_type::optional_repetition, symbol, input_position_ - 1};
  case '+':
    return token{token_type::required_repetition, symbol, input_position_ - 1};
  case '?':
    return token{token_type::option, symbol, input_position_ - 1};
  case '\\': {
    if (input_position_ >= input_.size())
  ...
  default:
    return token{token_type::character, symbol, input_position_ - 1};
  }
}
