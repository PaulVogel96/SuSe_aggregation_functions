...
class summary_selector_mult : public summary_selector_base<counter_type> {
  public:
    summary_selector_mult(std::string_view query, std::size_t summary_size, std::size_t time_window_size, std::size_t time_to_live = std::numeric_limits<std::size_t>::max())
        : summary_selector_base<counter_type>(query, summary_size, time_window_size, time_to_live),
          total_mult_counter_{this->automaton_.number_of_states()},
          total_detected_mult_counter_{this->automaton_.number_of_states()},
          active_window_mult_extension_{create_additional_window_info(time_window_size)} {
    
        //multiplication counters must be initialzed with 1
        std::fill(total_mult_counter_.begin(), total_mult_counter_.end(), 1);
        std::fill(total_detected_mult_counter_.begin(), total_detected_mult_counter_.end(), 1);
    }
    ...
    void remove_event(std::size_t cache_index) override {
        ...
        this->total_mult_counter_ -= this->mult_cache_[cache_index].state_counter;
        ...
        this->mult_cache_.erase(this->mult_cache_.begin() + cache_index);

        if (this->cache_.empty()) {
            this->active_window_.start_idx = 0;
            this->reset_counters(this->active_window_);
            this->reset_additional_window_counters(this->active_window_mult_extension_);
            return;
        }
    ...
    void add_event(const event &new_event) override {
        ...
        auto global_change_mult = advance_mult(this->active_window_.total_counter, this->active_window_mult_extension_.total_mult_counter, this->per_character_edges_, new_event);
        ...
        this->active_window_mult_extension_.total_mult_counter *= global_change_mult;
        this->total_mult_counter_ *= global_change_mult;
        this->total_detected_mult_counter_ *= global_change_mult;
        ...
        for (std::size_t i = 0; i < active_window_size; ++i) {
            ...
            const auto local_change_mult = advance_mult(this->active_window_.per_event_counters[i], this->active_window_mult_extension_.per_event_mult_counters[i], this->per_character_edges_, new_event);
            this->mult_cache_[cache_idx].state_counter *= local_change_mult;
            this->active_window_mult_extension_.per_event_mult_counters[i] *= local_change_mult;
        }
        ...
        this->active_window_mult_extension_.per_event_mult_counters.push_back(global_change_mult);
        this->mult_cache_.emplace_back(new_event, std::move(global_change_mult));
    }
  ...
  private:
    ...
    struct window_info_mult_extension {
        execution_state_counter<counter_type> total_mult_counter;
        suse::ring_buffer<execution_state_counter<counter_type>> per_event_mult_counters;
        friend auto operator<=>(const window_info_mult_extension &, const window_info_mult_extension &) = default;
    };
    window_info_mult_extension active_window_mult_extension_;

    execution_state_counter<counter_type> total_mult_counter_, total_detected_mult_counter_;

    auto create_additional_window_info(std::size_t window_size) const {
        window_info_mult_extension wnd{
