...
TEST_SUITE("suse::summary_selector_sum function") {
    ...
    TEST_CASE("Sum function simple") {
        ...
        CHECK(selector.sum_of_contained_complete_matches() == 110);
    }

    TEST_CASE("Calculate mean correctly with float counters") {
        suse::summary_selector_sum<float> selector("a(b*c)*d", 10, 10);
        std::string stream = "a3b4a1b2c5d6";
        ...
        CHECK(selector.mean_of_contained_complete_matches() == 13.75);
    }
}