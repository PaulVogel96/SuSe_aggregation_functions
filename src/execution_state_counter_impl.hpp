...
namespace suse
{
	...
	template <typename underlying>
	execution_state_counter<underlying> advance(const execution_state_counter<underlying>& counter, const edgelist& per_character_edges, suse::event new_event)
	{
		auto followup = execution_state_counter<underlying>{counter.size()};
